...
export const PolicyList = () => {
  ...
  const fetchFabricAssociations = async (policyId) => {
    ...
    try {
      ...
      // Filter fabrics that match this policy's content
      const matchingFabrics = (fabrics || []).filter(
        f => f.shipping_returns === policy.content
      );

      // For each matching fabric, get product sync status
      const fabricsWithSyncStatus = await Promise.all(
        matchingFabrics.map(async (fabric) => {
          ...
          // Count products that match the fabric's shipping_returns
          const { count: syncedProducts } = await supabaseAdminClient
            .from("products")
            .select("*", { count: "exact", head: true })
            .eq("fabric_type", fabric.name)
      ...
      setFabricAssociations(prev => ({
        ...prev,
        [policyId]: fabricsWithSyncStatus,
      }));
    } catch (error) {
  ...
  const handleApplyToSelectedFabrics = async (policyId) => {
    ...
    if (!policy || selectedFabricIds.length === 0) {
      openNotification?.({
        ...
        description: "Please select at least one fabric to apply the policy to.",
      });
      return;
    }

    if (!window.confirm(
      `Apply "${policy.name}" to ${selectedFabricIds.length} selected fabric${selectedFabricIds.length > 1 ? 's' : ''}?\n\nThis will update the fabrics' shipping & returns field.`
    )) {
      return;
    }
    ...
    try {
      ...
      openNotification?.({
        type: "success",
        message: "Policy Applied to Fabrics",
        description: `Updated ${updatedCount} fabric${updatedCount !== 1 ? 's' : ''}`,
      });
              ...
              <Box flex={1}>
                ...
                <Stack spacing={0.5} component="ul" sx={{ pl: 2, mb: 0 }}>
                  <Typography variant="body2" component="li">
                    <strong>Create Templates:</strong> Build reusable policy texts
                  </Typography>
                  <Typography variant="body2" component="li">
                    <strong>Set Default:</strong> Auto-applies to new fabrics
                  </Typography>
                  <Typography variant="body2" component="li">
                    <strong>View Associations:</strong> See which fabrics use each policy
                  </Typography>
                  <Typography variant="body2" component="li">
                    <strong>Check Sync Status:</strong> Monitor product alignment (🟢 synced, 🟡 partial, 🔴 needs sync)
                  </Typography>
                  <Typography variant="body2" component="li">
                    <strong>Bulk Apply:</strong> Update multiple fabrics and sync their products
                  </Typography>
                </Stack>
              </Box>
            </Stack>
                    ...
                    >
                      <Stack direction="row" alignItems="center" spacing={2}>
                        <Typography variant="body1" fontWeight={600}>
                          Manage Fabrics Using This Policy
                        </Typography>
                        {isExpanded && fabrics.length > 0 && (
                          <Badge badgeContent={fabrics.length} color="primary">
                            <Chip 
                              label={`${fabrics.length} fabric${fabrics.length !== 1 ? 's' : ''}`}
                              size="small"
                              color="primary"
                              variant="outlined"
                            />
                      ...
                      {loadingAssociations ? (
                        ...
                        <Alert severity="info" sx={{ borderRadius: 2 }}>
                          <Typography variant="body2" fontWeight={500}>
                            No fabrics are currently using this policy content.
                          </Typography>
                          <Typography variant="body2" sx={{ mt: 1 }}>
                            Edit fabrics and select this policy from the dropdown to apply it.
                          </Typography>
                        </Alert>
                      ) : (
                        <Box>
                          ...
                          <Stack direction="row" spacing={2} mb={3}>
                            ...
                            >
                              Apply to {selectedCount || ''} Selected Fabric{selectedCount !== 1 ? 's' : ''}
                            </Button>
                            <Button
                              variant="outlined"
                              startIcon={<Sync />}
                          ...
                          <Paper variant="outlined" sx={{ borderRadius: 2 }}>
                            ...
                            >
                              ...
                              <Typography variant="caption" fontWeight={700} sx={{ minWidth: 180, textAlign: "right", textTransform: "uppercase", letterSpacing: 1 }}>
                                Product Sync Status
                              </Typography>
                            </Box>
                                  ...
                                  <Box sx={{ minWidth: 180, textAlign: "right" }}>
                                    {!hasProducts ? (
                                      ...
                                      <Stack direction="row" alignItems="center" spacing={1.5} justifyContent="flex-end">
                                        <Tooltip title={`${fabric.syncedProducts} of ${fabric.totalProducts} products match this policy`} arrow>
                                          <Box display="flex" alignItems="center">
                                            {isSynced ? (
                                              <CheckCircle fontSize="small" sx={{ color: "success.main" }} />
                                            ) : fabric.syncPercentage > 50 ? (
                                        ...
                                        <Box sx={{ width: 80 }}>
                                          <LinearProgress
                                            variant="determinate"
                                            value={fabric.syncPercentage}
                                            color={isSynced ? "success" : fabric.syncPercentage > 50 ? "warning" : "error"}
                                            sx={{ height: 8, borderRadius: 4 }}
                                          />
                                        </Box>
                                        <Typography variant="caption" fontWeight={600} sx={{ minWidth: 40 }}>
                                          {fabric.syncPercentage}%
                                        </Typography>
                                      </Stack>
                                    )}
                                  </Box>
